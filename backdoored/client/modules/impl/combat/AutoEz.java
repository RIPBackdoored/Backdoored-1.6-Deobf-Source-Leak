...
public class AutoEz extends c
{
    ...
    public void a(final AttackEntityEvent v) {
        if (this.bu() && v.getTarget() instanceof EntityPlayer) {
            ...
            if (v.getEntityPlayer().getUniqueID().equals(AutoEz.mc.player.getUniqueID())) {
                if (v2.getHealth() <= 0.0f || v2.isDead || !AutoEz.mc.world.playerEntities.contains(v2)) {
                    AutoEz.mc.player.sendChatMessage(this.pc.ci());
                    y();
                    return;
                }
                this.pn = 500;
    ...
    public void bh() {
        ...
        if (this.pn > 0 && (this.entityPlayer.getHealth() <= 0.0f || this.entityPlayer.isDead || !AutoEz.mc.world.playerEntities.contains(this.entityPlayer))) {
            if (this.bu()) {
                AutoEz.mc.player.sendChatMessage(this.pc.ci());
                y();
            }
    ...
    public void a(final z v) {
        ...
        if (!v.bq().equals((Object)AutoEz.mc.player) && v.bq().equals((Object)this.entityPlayer)) {
            AutoEz.mc.player.sendChatMessage(this.pc.ci());
            y();
        }
    }
