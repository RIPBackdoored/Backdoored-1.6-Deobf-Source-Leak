...
public class AutoCrystal extends c
{
    private f.b.f.c ko;
    private f.b.f.c kt;
    private f.b.f.c kw;
    ...
    private void gf() {
        this.blockPos = null;
        this.entity = null;
        final boolean v = !this.kh.cq() || AutoCrystal.mc.player.getActiveItemStack().getItem() != Items.GOLDEN_APPLE;
        final boolean v2 = !this.kv.cq() || AutoCrystal.mc.player.getActiveItemStack().getItem() != Items.DIAMOND_PICKAXE;
        if (!v || !v2) {
            return;
        }
        final List<BlockPos> v3 = this.gk();
        final List<EntityPlayer> v4 = new ArrayList<EntityPlayer>();
        for (final EntityPlayer v5 : AutoCrystal.mc.world.playerEntities) {
            if (!f.b.q.x.c.g(v5)) {
                v4.add(v5);
            }
        }
        ...
        for (final EntityPlayer v9 : v4) {
            if (!v9.getUniqueID().equals(AutoCrystal.mc.player.getUniqueID())) {
                if (v9.isDead) {
                    continue;
                }
                for (final BlockPos v10 : v3) {
                    ...
                    final double v12 = a(v10.getX() + 0.5, v10.getY() + 1, v10.getZ() + 0.5, (Entity)AutoCrystal.mc.player) / 10.0f;
                    if (v11 < this.pa.ck()) {
                        continue;
                    }
                    boolean v13 = true;
                    final RayTraceResult v14 = AutoCrystal.mc.world.rayTraceBlocks(new Vec3d(AutoCrystal.mc.player.posX, AutoCrystal.mc.player.posY + AutoCrystal.mc.player.getEyeHeight(), AutoCrystal.mc.player.posZ), new Vec3d(v10.getX() + 0.5, v10.getY() - 0.5, v10.getZ() + 0.5));
                    v13 = ((v14 != null && v14.typeOfHit == RayTraceResult.Type.BLOCK) || AutoCrystal.mc.player.getDistance((double)v10.getX(), (double)v10.getY(), (double)v10.getZ()) <= this.kj.ck());
                    final boolean v15 = AutoCrystal.mc.player.getDistance((double)v10.getX(), (double)v10.getY(), (double)v10.getZ()) <= this.kd.ck();
                    if (!v15 || !v13) {
                        continue;
                    }
                    if (v11 > v6) {
        ...
        final boolean v16 = AutoCrystal.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL || AutoCrystal.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL;
        if (!v16 && !this.ku.cq()) {
            return;
        }
        if (v16) {
            final RayTraceResult v17 = AutoCrystal.mc.world.rayTraceBlocks(new Vec3d(AutoCrystal.mc.player.posX, AutoCrystal.mc.player.posY + AutoCrystal.mc.player.getEyeHeight(), AutoCrystal.mc.player.posZ), new Vec3d(v8.getX() + 0.5, v8.getY() - 0.5, v8.getZ() + 0.5));
            EnumFacing v18;
            ...
            AutoCrystal.mc.playerController.processRightClickBlock(AutoCrystal.mc.player, AutoCrystal.mc.world, v8, v18, v20, v19);
            AutoCrystal.mc.player.swingArm(v19);
            return;
        }
        final int v21 = n.b(Items.END_CRYSTAL);
        if (v21 >= 0) {
            AutoCrystal.mc.player.inventory.currentItem = v21;
        }
    }
    
    private void gq() {
        final EntityEnderCrystal v = (EntityEnderCrystal)AutoCrystal.mc.world.loadedEntityList.stream().filter(\u0000u::b).map(\u0000u::a).min(Comparator.comparing((Function<? super T, ? extends Comparable>)\u0000u::a)).orElse(null);
        if (v != null) {
            final double v2 = AutoCrystal.mc.player.getDistance((Entity)v);
            boolean v3 = true;
            final RayTraceResult v4 = AutoCrystal.mc.world.rayTraceBlocks(new Vec3d(AutoCrystal.mc.player.posX, AutoCrystal.mc.player.posY + AutoCrystal.mc.player.getEyeHeight(), AutoCrystal.mc.player.posZ), new Vec3d(v.posX + 0.5, v.posY - 0.5, v.posZ + 0.5));
            ...
            if (v5 && v3 && System.nanoTime() / 1000000L - this.pr >= 250L) {
                AutoCrystal.mc.playerController.attackEntity((EntityPlayer)AutoCrystal.mc.player, (Entity)v);
    ...
    private List<BlockPos> gk() {
        ...
        v.addAll((Collection)this.a(new BlockPos(Math.floor(AutoCrystal.mc.player.posX), Math.floor(AutoCrystal.mc.player.posY), Math.floor(AutoCrystal.mc.player.posZ)), (float)this.kd.ck(), (int)Math.round(this.kd.ck()), false, true, 0).stream().filter((Predicate<? super Object>)this::a).collect((Collector<? super Object, ?, List<? super Object>>)Collectors.toList()));
    ...
    private boolean a(final BlockPos v) {
        ...
        return (AutoCrystal.mc.world.getBlockState(v).getBlock() == Blocks.BEDROCK || AutoCrystal.mc.world.getBlockState(v).getBlock() == Blocks.OBSIDIAN) && AutoCrystal.mc.world.getBlockState(v2).getBlock() == Blocks.AIR && AutoCrystal.mc.world.getBlockState(v3).getBlock() == Blocks.AIR && AutoCrystal.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(v2)).isEmpty();
    }
    ...
    private static float a(final double v, final double v, final double v, final Entity v) {
        ...
        if (v instanceof EntityLivingBase) {
            v8 = a((EntityLivingBase)v, a(v7), new Explosion((World)AutoCrystal.mc.world, (Entity)null, v, v, v, 6.0f, false, true));
        }
        return (float)v8;
    }
    ...
    private static float a(final float v) {
        final int v2 = AutoCrystal.mc.world.getDifficulty().getId();
        return v * ((v2 == 0) ? 0.0f : ((v2 == 2) ? 1.0f : ((v2 == 1) ? 0.5f : 1.5f)));
    }
    
    private static /* synthetic */ Float a(final EntityEnderCrystal v) {
        return AutoCrystal.mc.player.getDistance((Entity)v);
    }
