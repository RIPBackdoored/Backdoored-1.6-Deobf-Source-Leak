...
public class FakeItem extends c
{
    private int xf;
    private int xq;
    ...
    public void bv() {
        FakeItem.mc.player.connection.sendPacket((Packet)new CPacketHeldItemChange(0));
    }
    
    public void bd() {
        FakeItem.mc.player.connection.sendPacket((Packet)new CPacketHeldItemChange(FakeItem.mc.player.inventory.currentItem));
    }
    
    private int gw() {
        for (int v = 0; v < 9; ++v) {
            final ItemStack v2 = (ItemStack)FakeItem.mc.player.inventory.mainInventory.get(v);
