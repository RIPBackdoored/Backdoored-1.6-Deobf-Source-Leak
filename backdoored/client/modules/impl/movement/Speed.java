...
public class Speed extends c
{
    ...
    private void a(final double v, final float v, final double v) {
        final boolean v2 = (!this.es.cq() && Speed.mc.player.moveForward != 0.0f) || Speed.mc.player.moveForward > 0.0f;
        if (v2 || Speed.mc.player.moveStrafing != 0.0f) {
            Speed.mc.player.setSprinting(true);
            if (Speed.mc.player.onGround) {
                Speed.mc.player.motionY = v;
                final float v3 = h.ny();
                ...
                final double v4 = Math.sqrt(Speed.mc.player.motionX * Speed.mc.player.motionX + Speed.mc.player.motionZ * Speed.mc.player.motionZ);
                final double v5 = h.ny();
                Speed.mc.player.motionX = -Math.sin(v5) * v * v4;
                Speed.mc.player.motionZ = Math.cos(v5) * v * v4;
            }
        }
    }
