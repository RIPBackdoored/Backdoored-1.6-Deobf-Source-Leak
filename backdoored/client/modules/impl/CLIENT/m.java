    ...
    public void a(final DrawBlockHighlightEvent v) {
        if (this.bu()) {
            ...
            if (v4.typeOfHit == RayTraceResult.Type.BLOCK) {
                ...
                final IBlockState v6 = m.mc.world.getBlockState(v5);
                if (v6.getMaterial() != Material.AIR && m.mc.world.getWorldBorder().contains(v5)) {
                    ...
                    b.drawSelectionBoundingBox(v6.getSelectedBoundingBox((World)m.mc.world, v5).grow(0.0020000000949949026).offset(-v7, -v8, -v9), (float)Math.min(Math.abs(this.is.cp() - 255), 244), (float)Math.min(Math.abs(this.ie.cp() - 255), 244), (float)Math.min(Math.abs(this.ix.cp() - 255), 244), (float)this.iz.ck());
                }
