...
public class ChatBot extends c
{
    private u in;
    private long il;
    ...
    private void c(final String v, final String v) {
        if (ChatBot.mc.player == null || v.startsWith("<" + ChatBot.mc.player.getName()) || v.startsWith("<" + ChatBot.mc.player.getDisplayNameString())) {
            return;
        }
        try {
            if (this.in == null) {
            ...
            if (v2 != null) {
                ChatBot.mc.player.sendChatMessage(v2);
            }
        }
        catch (Exception v3) {
            this.a(false);
