...
public class e extends c
{
    ...
    public void bv() {
        this.a(false);
        e.mc.world.sendQuittingDisconnectingPacket();
        e.mc.loadWorld((WorldClient)null);
        if (e.mc.isIntegratedServerRunning()) {
            e.mc.displayGuiScreen((GuiScreen)new GuiMainMenu());
        }
        else if (e.mc.isConnectedToRealms()) {
            final RealmsBridge v = new RealmsBridge();
            v.switchToRealms((GuiScreen)new GuiMainMenu());
        }
        else {
            e.mc.displayGuiScreen((GuiScreen)new GuiMultiplayer((GuiScreen)new GuiMainMenu()));
        }
        y();
    }
