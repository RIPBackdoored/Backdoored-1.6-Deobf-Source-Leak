...
public class LogoutSpots extends c
{
    ...
    public void a(final bb v) {
        final EntityPlayer v2 = LogoutSpots.mc.world.getPlayerEntityByUUID(v.gd.getId());
        if (v2 != null && LogoutSpots.mc.player != null && !LogoutSpots.mc.player.equals((Object)v2)) {
    ...
    public void a(final TickEvent.ClientTickEvent v) {
        if (LogoutSpots.mc.world == null && this.ft.size() != 0) {
            this.ft.clear();
            y();
        }
    }
    ...
    private static /* synthetic */ void a(final Color v, final String v, final AxisAlignedBB v) {
        ...
        if (LogoutSpots.mc.player.getDistanceSq(v2.x, v2.y, v2.z) > 2500.0) {
            final Vec3d v3 = v2.subtract(new Vec3d(LogoutSpots.mc.getRenderManager().viewerPosX, LogoutSpots.mc.getRenderManager().viewerPosY, LogoutSpots.mc.getRenderManager().viewerPosZ)).normalize();
            v2 = new Vec3d(LogoutSpots.mc.getRenderManager().viewerPosX + v3.x * 50.0, LogoutSpots.mc.getRenderManager().viewerPosY + v3.y * 50.0, LogoutSpots.mc.getRenderManager().viewerPosZ + v3.z * 50.0);
        }
        double v4 = LogoutSpots.mc.player.getDistance(v2.x, v2.y, v2.z) / 4.0;
        v4 = Math.max(1.6, v4);
        final RenderManager v5 = LogoutSpots.mc.getRenderManager();
        GL11.glPushMatrix();
        ...
        final String v6 = v + " (" + LogoutSpots.mc.player.getDistance(v.getCenter().x, v.getCenter().y, v.getCenter().z) + "m)";
        final int v7 = LogoutSpots.mc.fontRenderer.getStringWidth(v6) / 2;
        LogoutSpots.mc.fontRenderer.drawStringWithShadow(v6, (float)(-v7), (float)(-(LogoutSpots.mc.fontRenderer.FONT_HEIGHT - 1)), v.getRGB());
        GL11.glDisable(3042);
        GL11.glEnable(2896);
        GL11.glPopMatrix();
